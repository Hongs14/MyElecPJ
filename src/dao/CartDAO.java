...
/*		// 장바구니에 담겨있는 상품 총액 읽어오기
	...
	// 장바구니 상품 이미지
	public CartDTO getImage(String user_id, int product_id, Connection conn) throws SQLException {
		String sql = "select product_filename, product_savedname, product_contenttype from product p, cart_detail cd where cd.product_id=p.product_id and cd.users_id=? and cd.product_id=? ";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, user_id);
		pstmt.setInt(2, product_id);
		ResultSet rs = pstmt.executeQuery();

		CartDTO cartImage = new CartDTO();
		while (rs.next()) {
			cartImage.setProduct_filename(rs.getString("product_filename"));
			cartImage.setProduct_savedname(rs.getString("product_savedname"));
			cartImage.setProduct_contenttype(rs.getString("product_contenttype"));
			System.out.println(cartImage.getProduct_filename());
		}
		rs.close();
		pstmt.close();
